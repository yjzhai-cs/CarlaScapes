...
def sensor_callback(sensor_data, sensor_queue, sensor_name):
    if 'lidar' in sensor_name:
        sensor_data.save_to_disk(os.path.join('../outputs/output_synchronized', '%06d_%06d.ply' % (sensor_data.timestamp, sensor_data.frame)))
    if 'camera' in sensor_name:
        sensor_data.save_to_disk(os.path.join('../outputs/output_synchronized', '%06d_%06d.png' % (sensor_data.timestamp, sensor_data.frame)))
...
def main():
    ...
    try:
        ...
        ego_vehicle_bp = blueprint_library.find('vehicle.citroen.c3')
        # black color
